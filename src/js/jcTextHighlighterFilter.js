                ...
                if (textToFilter.length > 0) {
                    if(settings.hideNegatives) {
                        if(settings.animateHideNShow) {
                            jQuery('.'+settings.parentLookupClass).closest('.panel').stop(true, true).fadeOut('slow');
                        }
                        else {
                            jQuery('.'+settings.parentLookupClass).closest('.panel').stop(true, true).hide();
                        }
                    }
                    var _cs = "icontains";
                    if(settings.caseSensitive) {
                    ...
                    jQuery.each(jQuery('.'+settings.childBlockClass+":"+_cs+"(" + textToFilter + ")"),function(i,obj) {
                        if(settings.hideNegatives) {
                            if(settings.animateHideNShow) {
                                jQuery(obj).closest('.panel').stop(true, true).fadeIn('slow');
                            }
                            else {
                                jQuery(obj).closest('.panel').stop(true, true).show();
                            }
                    ...
                    if(settings.hideNegatives) {
                        if(settings.animateHideNShow) {
                            jQuery('.'+settings.parentLookupClass).closest('.panel').stop(true, true).fadeIn('slow');
                        }
                        else {
                            jQuery('.'+settings.parentLookupClass).closest('.panel').stop(true, true).show();
                        }
                    }
                }
            }
